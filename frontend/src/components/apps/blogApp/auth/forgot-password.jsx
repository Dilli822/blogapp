...
import { Layout, Form, Input, Button, Typography, Col, Row, message, Alert } from 'antd';
...
const ForgotPassword = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/account/password-reset-email/", {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <AppHeader />
      <Layout style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
        <Row>
            ...
            <div style={{ padding: '20px', backgroundColor: '#fff', borderRadius: '10px', boxShadow: '3px 6px 5px rgba(0, 0, 0, 0.1)' }}>
              {!emailSent ? (
                ...
                >
                  ...
                  >
                    <Input prefix={<MailOutlined className="site-form-item-mail" />} placeholder="Email" disabled={submitting} />
                  </Form.Item>
                  <Form.Item>
                    <Button type="primary" htmlType="submit" className="forgot-password-form-button" style={{ minWidth: '100%' }} disabled={submitting}>
